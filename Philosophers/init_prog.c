...
void	init_right_fork(t_prog **prog)
{
	t_philo	*philo;
	t_philo	*next_philo;
	int		i;
	...
	{
		philo = &(*prog)->threads[i];
		next_philo = &(*prog)->threads[(i + 1) % (*prog)->phil_num];
		philo->right_fork = next_philo->left_fork;
	}
}

void	init_threads(t_prog **prog)
{
	int	i;
